...
export default function Layout(props) {
    ...
    return (
        <main className="main flex w-[var(--vvw)] overflow-hidden h-[var(--vvh)]">
                    ...
                    <div className="p-2 flex flex-col space-y-2">
                        {username && userId ? (
                            <div className='flex flex-col md:flex-row items-center w-full space-y-2 md:space-y-0'>
                                <span className="mr-2"><ProfilePicture avatar={avatar} setAvatar={setAvatar}
                                                                       username={username} userId={userId}/></span>
                                <span className="w-full"><Username username={username} userId={userId}
                                                                   setUsername={setUsername}/></span>
                            </div>
                        ) : (
                            null
                        )}
            ...
            {/* Messages */}
            <div
                className="fixed md:relative z-[1] w-[var(--vvw] flex-1 bg-slate-200 dark:bg-gray-800 text-black dark:text-white"
                style={{height: 'var(--vvh)'}}>{props.children}</div>
        </main>
    )
}
            ...
            >
                ...
                {channel.id !== 1 && (channel.created_by === user?.id || userRoles.includes('admin')) && (
                    <ControlledMenu
                        ...
                        <MenuItem onClick={() => deleteChannel(channel.id)}>
                            <FaTrash /><span className='ml-2'>Delete Channel</span>
                        </MenuItem>
                    </ControlledMenu>
                )}
            </Link>
